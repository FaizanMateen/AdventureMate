...
exports.login = catchAsync(async (req, res, next) => {
  ...
  if (user.lockUntil && user.lockUntil > Date.now()) {
    ...
    return next(
      new AppError(
        `The account is locked please try after ${lockTime} minute`,
        401
      )
    );
  }
  // 4) Check if password is correct
  if (!(await user.correctPasswords(password, user.password))) {
    user.increamentLoginAttempts();
    return next(new AppError('Email or Password is incorrect!', 401));
  }
  // 5) If login is successful, reset login attempts and lockUntil
